...
namespace QuanLyKho.Views
{
    public partial class NhapHang : Form
    {
        ...
        CTPNCtrl ctctrl = new CTPNCtrl();
        public void addMaPN()
        {
            conn.OpenConn();
            string que = "select count (*) from PhieuNhap";
        ...
        private void add_data_hd(PhieuNhapObj pnObj)
        {
            pnObj.Mapn = txt_mapn.Text.Trim();
            pnObj.Ngaylap = dtp_ngaynhap.Value.ToShortDateString();
            pnObj.Tennv = cbb_nhanvien.SelectedValue.ToString();
            pnObj.Ncc = Int32.Parse(cbb_ncc.SelectedValue.ToString());
        ...
        private void bt_luu_Click(object sender, EventArgs e)
        {
            ...
            add_data_hd(hdObj);
            if (pnctrl.AddData(hdObj))
            {
                if (ctctrl.AddData(txt_mapn.Text, dtDSHH) && hhctrl.UpdSL(dtDSHH))
